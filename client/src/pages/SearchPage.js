...
const SearchPage = () => {
  ...
  const fetchData = (resetPage = false) => {
    ...
    const [sorting, descending] =
      getSorting.current === null ? ["price", true] : getSorting.current();
    ...
    get(`/api/ads/${entity}/search?page=${currentPage}${query}`).then(
      (response) => {
  ...
  return (
    <div
      key={location.key}
      className="py-20 px-2 sm:px-14 md:px-20 lg:px-32 xl:px-60 bg-silver-accent min-h-screen"
    >
      <SearchAndFilter
        whereProp={where ? where : ""}
        guestsProp={guests ? guests : ""}
        getParams={getParams}
        entityProp={entity}

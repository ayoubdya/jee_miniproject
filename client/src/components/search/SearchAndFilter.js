...
const SearchAndFilter = ({
  ...
  const handleDropdown = () => {
    if (
      isDropdownOpen &&
      document.activeElement === document.getElementById("dropdown-btn-search")
    ) {
  ...
  const getIconBasedOnType = () => {
    if (selectedEntity.includes("resorts")) return "tabler:window";
    else if (selectedEntity.includes("boats")) return "tabler:sailboat";
    else if (selectedEntity.includes("adventures")) return "tabler:fish";
  };
  ...
  return (
    <div className="w-full text-left hidden">
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h1 className="text-2xl text-left text-gray-400 font-sans font-light leading-4">
          Parameters
        </h1>
        ...
        <div className="flex flex-wrap gap-x-2 gap-y-2 mt-3">
            ...
            <div className="flex flex-col h-10">
              <div className="dropdown my-auto h-8" id="12341234123">
                <button
                  ...
                  id="dropdown-btn-search"
                  onClick={() => handleDropdown()}
                  onBlur={(e) => setIsDropdownOpen(false)}
                >
                  <Icon
                    className="h-8 w-8 p-1.5 text-gray-50 bg-slate-600 rounded-md transition ease-in-out hover:scale-105"
                    icon={getIconBasedOnType()}
                    inline={true}
                  />
                </button>
                <ul
                  tabIndex="0"
                  className="dropdown-content menu p-2 shadow bg-slate-600 text-left rounded-box w-40"
                >
          ...
          <div className="relative my-auto h-10">
            {guests && (
              <div className="absolute top-0.5 text-xs w-20 text-center font-thin text-slate-400 z-10">
                guests:
              </div>
            )}
            <input
          ...
          <div className="relative my-auto">
            {startDate && (
              <div className="absolute top-0.5 text-xs w-28 text-center font-thin text-slate-400 z-10">
                from:
              </div>
            )}
            <DatePicker
        ...
        <div className="flex flex-wrap w-full justify-between mt-4">
          ...
          <div className="flex self-end gap-x-3 mt-2 sm:mt-0">
            ...
            px-3 sm:px-6 py-1 bg-silver-accent hover:bg-slate-300"
            >
              Clear all
            </button>
            <button

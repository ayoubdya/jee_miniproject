...
const ClientReservationUpcoming = ({ data }) => {
  ...
  const fetchData = (resetPage = false) => {
    ...
    get(
      `/api/customers/upcoming-reservations?page=${currentPage}&sorting=${sorting}`
    ).then((response) => {
  ...
  return (
    <div>
          ...
          {reservations.map((reservation) => (
            <div key={reservation.id}>
              <ClientReservationItem
                ...
                allowCancel={
                  moment(reservation.startDateTime, "DD/MM/yyyy hh:mm").diff(
                    moment(),
                    "hours"
                  ) > 72 && !reservation.cancelled
                }
      ...
      {showConfirmationModal && (
        <ConfirmationModal
          closeFunction={() => setShowConfirmationModal(false)}
      ...
      {showMessageModal && (
        <MessageModal
          okayFunction={() => window.location.reload()}

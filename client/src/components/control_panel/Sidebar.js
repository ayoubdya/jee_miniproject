...
const Sidebar = ({ currentComponent, setCurrentComponent }) => {
  ...
  if (session.accountType === "SUPERUSER") {
    sidebarItems.push(
      ...
      // {
      //   name: "resolveRegistrationRequests",
    ...
    sidebarItems.push(
      {
        name: "myEntities",
        text: `My ${getNewEntityName()}s`,
        icon: getMyEntityIcon(),
      },
      {
        name: "newEntity",
        text: `New ${getNewEntityName()}`,
  ...
  return (
    <div className="block fixed z-20 mt-10">
      ...
      <div
        className="h-screen fixed z-50 bg-gray-100 shadow md:h-full flex-col justify-between md:hidden transition duration-150 ease-in-out"
        id="mobile-nav"
      >
        {!sidebarOpen ? (
          <div
            className="h-10 w-10 bg-gray-800 absolute right-0 mt-16 -mr-10 flex items-center shadow rounded-tr rounded-br justify-center cursor-pointer text-white"
          ...
          <div
            id="closeSideBar"
            className="md:hidden h-10 w-10 bg-gray-800 absolute right-0 mt-16 -mr-10 flex items-center shadow rounded-tr rounded-br justify-center cursor-pointer text-white"
        ...
        <div className="mt-6 space-y-1">
          <div className="mt-6 space-y-1">
            {sidebarItems.map((item) => {
              if (
                (item.name === "pendingReport" ||
                  item.name === "activeReservations") &&
                currentComponent !== "reservations" &&
                currentComponent !== "pendingReport" &&

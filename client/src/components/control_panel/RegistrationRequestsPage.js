...
const RegistrationRequestsPage = () => {
  ...
  accountTypeIconMap.set("RESORT_OWNER", {
    icon: "tabler:home-2",
    name: "Resort owner",
  });
  accountTypeIconMap.set("BOAT_OWNER", {
    icon: "tabler:speedboat",
    name: "Jetski owner",
  });
  accountTypeIconMap.set("FISHING_INSTRUCTOR", {
    icon: "tabler:fish",
    name: "Fishing instructor",
  });

  useEffect(() => {
    get(`/api/admin/registration-requests/`).then((response) => {
            ...
            <div className="grid grid-cols-10 pt-3">
              <div className="block col-span-7">
                ...
                <div className="flex mt-2 text-gray-600 text-sm">
                  <Icon
                    className="mr-2"
                    icon={accountTypeIconMap.get(request.accountType).icon}
                    inline={true}
                    width="20"
                  />
                  <span className="mr-4">
                    {accountTypeIconMap.get(request.accountType).name}
                  </span>
                  <Icon
                    className="mr-2"
                    icon="tabler:clock"
                  ...
                  />
                  <span className="mr-4">
                    {Moment(request.createdAt).format("D MMM YYYY HH:mm")}
                  </span>
                </div>
                <div className="text-left mt-4">{request.explanation}</div>
              ...
              <div className="flex col-span-3">
                <button
                  className="bg-gray-300 hover:bg-gray-400 active:bg-gray-500 drop-shadow-md
              text-gray-700 rounded-lg py-2.5 lg:py-2 text-sm lg:text-base h-min w-20 mb-1.5 mt-3 md:mt-0"
                ...
                <button
                  className="bg-teal-600 hover:bg-teal-700 active:bg-teal-800 drop-shadow-md
